...
import { applyVoiceProfile, matchingVoiceProfile, VOICE_PROFILES } from '../../shared/voice-profiles'
...
const sliders = [
  ...
  ['echoMix', 'Echo amount', 0, 0.3, 0.01], ['echoDelayMs', 'Echo delay · ms', 60, 500, 10], ['echoRepeats', 'Echo repeats', 1, 4, 1],
] as const
export function VoiceStudio({ avatar, model, disabled, bridge, onChange }: {
  ...
  const effects = useMemo(() => ({ ...DEFAULT_VOICE_EFFECTS, ...avatar.voiceEffects }), [avatar.voiceEffects])
  ...
  const voiceProfile = matchingVoiceProfile(avatar)
  return <section className="voice-studio" aria-label="Voice Studio">
    <div className="voice-studio__layout"><div>
      ...
      <fieldset disabled={disabled}><legend>Local voice effects</legend><div className="console__action-row">
        ...
        <HelpField help={FIELD_HELP.roomSize}>Room size<select value={effects.roomSize} onChange={e => edit({ roomSize: e.currentTarget.value as VoiceEffects['roomSize'] })}><option value="short">Short · 120 ms</option><option value="medium">Medium · 250 ms</option><option value="hall">Hall · 1.2 s</option><option value="cathedral">Cathedral · 2.4 s</option></select></HelpField>
        <p className="console__muted">Room adds a diffuse reverberation. Echo adds distinct fading repeats; set Echo amount to 0 to turn them off. Stop or interruption clears both immediately.</p>
