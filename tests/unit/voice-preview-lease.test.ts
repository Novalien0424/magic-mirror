...
describe('voice preview output lease', () => {
  ...
  it('live preemption rejects a late credential result before any output is exposed', async () => {
    ...
    await vi.waitFor(() => expect(complete).toBeTypeOf('function')); x.lease.preempt(); x.active(); complete({ value: 'synthetic-ephemeral' })
  ...
  it('cancels pending generation immediately and allows another audition before credentials arrive', async () => {
    const x = setup()
    let complete!: (value: { value: string }) => void
    x.broker.issue.mockImplementation(() => new Promise(resolve => { complete = resolve }))
    const pending = x.lease.acquire({ ...request, kind: 'generated' })
    await vi.waitFor(() => expect(complete).toBeTypeOf('function'))
    expect(x.lease.cancelPending()).toBe(true)
    expect(await pending).toEqual({ ok: false, reason: 'voice_preview_preempted' })
    const next = await x.lease.acquire(request)
    expect(next.ok).toBe(true)
    x.enableOutput.mockClear()
    complete({ value: 'synthetic-ephemeral' })
    await Promise.resolve()
    expect(x.enableOutput).not.toHaveBeenCalled()
    if (next.ok) x.lease.release(next.token)
  })
})
