---
name: mm-wake-word
description: Use when implementing or tuning the sherpa-onnx Chinese wake-word worker, custom keyword encoding, mic capture for the wake path, false-wake tuning, or the wake↔realtime mic handoff (Phase 2).
---
...
Verified **2026-08-16**. Baseline stack: `sherpa-onnx-node@1.13.5` +
`sherpa-onnx-kws-zipformer-wenetspeech-3.3M-2024-01-01` (int8) + `decibri`
capture, in a worker process owned by Electron Main. Re-verify versions when
Phase 2 pins the lockfile.
...
- Keywords workflow: human `keywords_raw.txt` → encoded `keywords.txt` via
  ...
  `<phrase> :<boost> #<threshold> @<display>` — e.g. `魔鏡魔鏡 :2.0 #0.45 @魔鏡魔鏡`
  (display text may not contain spaces; use `_`). Per-keyword values of
  0/absent fall back to globals.
...
- Wake phrase: 3–6 syllables, not a daily-conversation string (PRD §15).
  Validate with the recorded-WAV corpus runner in Console (Phase 2) and the
  30-min ambient/TV negative test.
...
- TCC attributes the mic prompt to the nearest signed ancestor (the app that
  ...
  denial, surface it as a Console `wake_worker` failure, don't spin.
...
- Worker crash → Main restarts once; still failing → Maintenance + Console
  `Failed` (Spec §14). Wake must keep working with the network down.
